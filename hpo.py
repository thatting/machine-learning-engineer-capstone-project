...
def test(model, test_loader, criterion):
    ...
    running_loss=0
    running_corrects=0
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # Added 270724: Check if CUDA is available  
    for inputs, labels in test_loader:
        inputs = inputs.to(device)  # Added 270724: Offload to GPU
        labels = labels.to(device)  # Added 270724: Offload to GPU
...
def train(model, train_loader, validation_loader, criterion, optimizer, epochs):
    '''
    This function takes a model and training dataloaders and then trains the model
    '''
    epochs=epochs
    best_loss=1e6
...
def net():
    '''
    This function initializes the model and adds an output layer corresponding to 5 classes
    '''
    ...
    model.fc = nn.Sequential(
                   ...
                   nn.Linear(128, 5))  # Set the number of outputs to 5
    return model


def create_data_loaders(data, batch_size):
    '''
    This function creates the data loaders for training, validation and testing
    '''
...
def main(args):
    ...
    model=net().to(device)  # Added 270724: Offload to GPU
    '''
    Create loss criterion and optimizer
    '''
    loss_criterion = nn.CrossEntropyLoss(ignore_index=5)  # Set value ignore_index to 5 corresponding to 5 classes
    optimizer = optim.Adam(model.fc.parameters(), lr=args.learning_rate)
    '''
    Call the training function to start model training
    '''
